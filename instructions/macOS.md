...
This guide shows you how to set up Microsoft SQL Server 2022 on your local macOS machine for use with Visual Studio Code. It works for Apple Silicon (M1, M2, etc.) as well as older Macs with Intel processors. Since there is no native way to install SQL Server on macOS, we will run it in a Docker container.
...
10. When finished, you can stop the Docker container via Docker Desktop. You do not need to use the terminal again. You can restart the container from Docker Desktop. If you encounter issues with the container, you may delete and recreate it. Your changes to SQL Server will be saved because of the volume.
